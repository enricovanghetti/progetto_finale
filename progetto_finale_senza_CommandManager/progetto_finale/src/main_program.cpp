...
int main() {
    ...
    //dispostivi ad accensione e spegnimento manuale
    ...
    manager.addDevice(std::make_shared<FCDevice>("Tapparelle elettriche", 10, 0.3, 1));
    //dispositivi a spenimento automaitco (presentano un parametro in piu' 'cycleDuration')
    manager.addDevice(std::make_shared<FCDevice>("Lavatrice", 2, 2.0, 110)); // 110 minuti
    manager.addDevice(std::make_shared<FCDevice>("Lavastoviglie", 3, 1.5, 195)); // 195 minuti
    manager.addDevice(std::make_shared<FCDevice>("Forno a microonde", 7, 0.8, 2)); // 2 minuti
    manager.addDevice(std::make_shared<FCDevice>("Asciugatrice", 9, 1.8, 60)); // 60 minuti
    manager.addDevice(std::make_shared<FCDevice>("Televisore", 8, 0.2, 60)); // 60 minuti
