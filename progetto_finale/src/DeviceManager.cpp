...
//costruttore: inizializza variabili e imposta lo stato iniziale
...
//controlla se c'è abbastanza potenza disponibile per accendere un nuovo dispositivo
bool DeviceManager::checkPowerForDevice(const std::shared_ptr<Device>& deviceToAdd) {
    if (isCheckingPower) return true; // Previene ricorsione infinita
    ...
    if (totalConsumption > maxPowerLimit) {
        ...
        //spegne dispositivi fino a rientrare nei limiti
...
//gestisce l'accensione/spegnimento dei dispositivi
